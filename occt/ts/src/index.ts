...
export class OcctKernel {
  ...
  static async init(options?: InitOptions): Promise<OcctKernel> {
    ...
    if (wasmSource instanceof ArrayBuffer || wasmSource instanceof Uint8Array) {
      ...
      const bytes =
        wasmSource instanceof Uint8Array
  ...
  makeArcEdge(start: Vec3, mid: Vec3, end: Vec3): ShapeHandle {
    return wrap('makeArcEdge', () =>
      handle(
        this.#raw.makeArcEdge(start.x, start.y, start.z, mid.x, mid.y, mid.z, end.x, end.y, end.z),
      ),
    );
  }
  ...
  translateBatch(shapes: ShapeHandle[], offsets: number[]): ShapeHandle[] {
    return wrap('translateBatch', () =>
      this.#withU32(shapes, (ids) =>
        this.#withF64(offsets, (off) => this.#vecToHandles(this.#raw.translateBatch(ids, off))),
      ),
    );
  }
  ...
  transformBatch(shapes: ShapeHandle[], matrices: number[]): ShapeHandle[] {
    return wrap('transformBatch', () =>
      this.#withU32(shapes, (ids) =>
        this.#withF64(matrices, (mats) => this.#vecToHandles(this.#raw.transformBatch(ids, mats))),
      ),
    );
  }
  ...
  rotateBatch(shapes: ShapeHandle[], params: number[]): ShapeHandle[] {
    return wrap('rotateBatch', () =>
      this.#withU32(shapes, (ids) =>
        this.#withF64(params, (p) => this.#vecToHandles(this.#raw.rotateBatch(ids, p))),
      ),
    );
  }
  ...
  scaleBatch(shapes: ShapeHandle[], params: number[]): ShapeHandle[] {
    return wrap('scaleBatch', () =>
      this.#withU32(shapes, (ids) =>
        this.#withF64(params, (p) => this.#vecToHandles(this.#raw.scaleBatch(ids, p))),
      ),
    );
  }
  ...
  mirrorBatch(shapes: ShapeHandle[], params: number[]): ShapeHandle[] {
    return wrap('mirrorBatch', () =>
      this.#withU32(shapes, (ids) =>
        this.#withF64(params, (p) => this.#vecToHandles(this.#raw.mirrorBatch(ids, p))),
      ),
    );
  }
  ...
  getSubShapes(
    shape: ShapeHandle,
    type: 'vertex' | 'edge' | 'wire' | 'face' | 'shell' | 'solid',
  ): ShapeHandle[] {
  ...
  downcast(
    shape: ShapeHandle,
    targetType: 'vertex' | 'edge' | 'wire' | 'face' | 'shell' | 'solid',
  ): ShapeHandle {
  ...
  faceTolerance(face: ShapeHandle): number {
    return wrap('faceTolerance', () => this.#raw.faceTolerance(face));
  }

  sectionByPlane(shape: ShapeHandle, point: Vec3, normal: Vec3): ShapeHandle {
    return wrap('sectionByPlane', () =>
      handle(
        this.#raw.sectionByPlane(shape, point.x, point.y, point.z, normal.x, normal.y, normal.z),
      ),
    );
  }

  edgeContinuity(edge: ShapeHandle, faceA: ShapeHandle, faceB: ShapeHandle): string {
    return wrap('edgeContinuity', () => this.#raw.edgeContinuity(edge, faceA, faceB));
  }

  distanceExtrema(shapeA: ShapeHandle, shapeB: ShapeHandle): Array<{ pointA: Vec3; pointB: Vec3 }> {
    return wrap('distanceExtrema', () => {
      const raw = this.#drainVector(this.#raw.distanceExtrema(shapeA, shapeB), Float64Array);
      const pairs: Array<{ pointA: Vec3; pointB: Vec3 }> = [];
      for (let i = 0; i + 5 < raw.length; i += 6) {
        pairs.push({
          pointA: { x: raw[i]!, y: raw[i + 1]!, z: raw[i + 2]! },
          pointB: { x: raw[i + 3]!, y: raw[i + 4]!, z: raw[i + 5]! },
        });
      }
      return pairs;
    });
  ...
  getFaceCylinderData(
    face: ShapeHandle,
  ): { radius: number; isDirect: boolean; location: Vec3; direction: Vec3 } | null {
