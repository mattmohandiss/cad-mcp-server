...
describe('QueryEngine: query_step dispatch', () => {
  ...
  it('routes faces queries through the engine and returns a queryOutputSchema envelope', async () => {
  ...
  it('routes edges queries through the engine', async () => {
    const data = await executeQuery({
      file_path: NIST_FILE,
      entities: 'edges',
      filter: { length_max: 5 },
...
describe('QueryEngine: query_step migrate parity', () => {
  it('returns the same envelope shape as the legacy 9-tool surface', async () => {
    const data = await executeQuery({
      file_path: NIST_FILE,
      entities: 'faces',
      limit: 1,
...
describe('PipelineExecutor: transact_step', () => {
  it('executes a single query step and returns the result', async () => {
  ...
  it('returns intermediate step results when return_intermediate is true', async () => {
    const result = await executePipeline({
      file_path: NIST_FILE,
      pipeline: [
  ...
  it('select projects the previous step result to specified fields', async () => {
    const result = await executePipeline({
      file_path: NIST_FILE,
      pipeline: [
        { op: 'query', params: { entities: 'faces', limit: 3 } },
  ...
  it('stages for_each / filter_results / walk_assembly with a limitations entry', async () => {
    const result = await executePipeline({
      file_path: ap242File,
      pipeline: [
        { op: 'walk_assembly', params: { per_node: [{ op: 'query', params: { entities: 'bodies' } }] } },
      ],
    });
