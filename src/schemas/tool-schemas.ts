...
export const filterSchema = z
  .object({
    ...
    area_min: z
      ...
      .optional()
      .describe(
        'Faces only. Minimum area in mm^2. Use to filter out chamfers/fillets when analyzing only primary faces.',
      ),
    area_max: z
      .number()
      .nonnegative()
      .optional()
      .describe('Faces only. Maximum area in mm^2. Use to find small faces (potential artifacts).'),
    normal: z
      .object({
    ...
    length_max: z
      ...
      .optional()
      .describe(
        'Edges only. mm. Use to find tiny edges (potential tessellation artifacts or short features under 0.5mm).',
      ),
    curvature_min: z
      .number()
      .nonnegative()
      .optional()
      .describe(
        'Edges only. 1/mm. Use with curvature_max to find edges within a curvature range (e.g., all fillets with radius 1-3mm => curvature 0.33-1.0).',
      ),
    ...
    has_curve3d: z
      ...
      .optional()
      .describe(
        'Edges only. True if the edge has a 3D curve representation. Some imported geometry only has 2D pcurves.',
      ),
    ...
    radius_min: z
      ...
      .optional()
      .describe(
        'Cylindrical faces or circular edges. mm. Use to find holes/fillets above a size threshold.',
      ),
    radius_max: z
      .number()
      .nonnegative()
      .optional()
      .describe('Cylindrical faces or circular edges. mm.'),
    body_ids: z
      .array(bodyIdSchema)
      .min(1)
      .optional()
      .describe(
        'Restrict to specific bodies. Required for multi-body models where queries must be scoped.',
      ),
    validity_status: z
      .enum(VALIDITY_STATUSES)
      .optional()
      .describe(
        'Faces, edges, or bodies. "self_intersecting" is the most common issue. Replaces the old boolean is_valid.',
      ),
    tolerance_max: z
      .number()
      .nonnegative()
      .optional()
      .describe(
        'Faces, edges, or vertices. mm. Tolerance is the precision of the B-rep representation. Values > 0.01mm often indicate lossy imports or STEP round-trips.',
      ),
    canonical_form: z
      .object({
      ...
      .optional()
      .describe(
        'Faces only. Recognize as a canonical shape (plane/cylinder/cone/sphere) within tolerance.',
      ),
    ...
    tolerance_subtype: z
      .enum(TOLERANCE_SUBTYPES)
      .optional()
      .describe('PMI only. Geometric tolerance type.'),
    value_min: z
      .number()
      .nonnegative()
      .optional()
      .describe(
        'PMI only. mm. Minimum tolerance value (for tolerances) or measurement value (for dimensions).',
      ),
    value_max: z.number().nonnegative().optional().describe('PMI only. mm.'),
    material_condition: z
      .enum(MATERIAL_CONDITIONS)
      .optional()
      .describe('PMI only. Material condition modifier.'),
    linked_to: z
      .object({
      ...
      .optional()
      .describe(
        'PMI only. Filter by the entity the PMI applies to. Supports nested filter on the linked entity.',
      ),
    ...
    is_instance: z
      ...
      .optional()
      .describe(
        'assembly_node only. True if the node is an instance (a placement of a part), false if it is a part definition.',
      ),
    is_root: z
      .boolean()
      .optional()
      .describe('assembly_node only. True if the node is at the top level of the assembly tree.'),
...
export const measureOpSchema = z
  .object({
    op: z
      .enum(MEASURE_OPS)
      .describe('The measurement operation. Each op has its own additional parameters below.'),
    /* ray */
    direction: direction3Schema
      .optional()
      .describe(
        'For ray tests and section_by_plane: the direction vector. Default [0,0,1] for Z-up. Does not need to be unit length; will be normalized.',
      ),
    origin: z
      .union([point3Schema, z.string()])
      .optional()
      .describe(
        'For ray_test_segment: where to start. "extent_max" = farthest point along the entity\'s extent in the +direction; "extent_min" = nearest. Or a fixed 3D point.',
      ),
    tmax: z
      .number()
      .positive()
      .optional()
      .describe('For ray_test_segment: maximum ray distance in mm. Default infinity.'),
    spacing_mm: z
      .number()
      .positive()
      .default(2.0)
      .describe(
        'For ray_test_grid: distance between grid rays in mm. Smaller = more samples, slower. Default 2.0.',
      ),
    /* distance */
    to: entityIdSchema
      .optional()
      .describe('For distance: target entity ID (e.g., "face:5", "edge:3", "body:0").'),
    /* section */
    plane_origin: point3Schema
      .optional()
      .describe('For section_by_plane: a point on the cutting plane.'),
    plane_normal: direction3Schema
      .optional()
      .describe('For section_by_plane: normal of the cutting plane.'),
    /* curvature */
    param: z
      .number()
      .min(0)
      ...
      .optional()
      .describe(
        'For curvature_at_param: parameter value along the curve (0 to 1, where 0 is the start and 1 is the end).',
      ),
    /* continuity */
    with: entityIdSchema
      .optional()
      .describe('For continuity: the other face/edge to check continuity with.'),
    /* point ops */
    point: point3Schema
      .optional()
      .describe('For classify_point / closest_point_on_face: 3D point to classify (x, y, z).'),
    tolerance: z
      .number()
      .nonnegative()
      .default(0.01)
      .describe(
        'For section_by_plane, canonical_form, and other tolerance-sensitive ops: tolerance in mm. Default 0.01.',
      ),
  })
  .strict();
...
export const aggregateSpec = z
  ...
  .regex(
    ...
    'Format: <op>[:<field>]. Examples: "count", "min:area", "max:radius", "avg:hit_distance".',
  )
  .describe(
    'Format: <op>[:<field>]. Ops: count (no field), min, max, avg, stddev, sum. Examples: "count", "min:area", "max:radius", "avg:hit_distance", "stddev:length", "sum:volume".',
  );
...
export const queryStepInputSchema = {
  ...
  entities: z
    .enum(ENTITIES)
    .describe(
      'Entity type to query. MUST be a single string — not an array. Valid values: "faces", "edges", "bodies", "vertices", "pmi", "color", "layer", "material", "assembly_node". Example: {"entities": "faces", ...}.',
    ),
  entity_ids: z
    .array(nonEmpty)
    .min(1)
    ...
    .optional()
    .describe(
      'Optional direct lookup. When provided, restricts the query to these specific entity IDs. Use when you already have IDs from a prior query.',
    ),
  filter: filterSchema
    .optional()
    .describe(
      'Filter entities by properties. The filter object accepts all fields; only fields relevant to the chosen entities are applied. Other fields are ignored.',
    ),
  group_by: z
  ...
  measure: z
    ...
    .optional()
    .describe(
      'Derived values to compute per entity or per group (ray tests, distances, curvature, section, continuity).',
    ),
  aggregate: z
    .array(aggregateSpec)
    .min(1)
    ...
    .optional()
    .describe(
      'Statistics to compute over the result set. Format: <op>:<field>. Examples: "min:area", "max:radius", "avg:hit_distance", "count:hit_distance". When set, intermediate entities are hidden unless return_intermediate is true.',
    ),
  select: z
    .array(z.string())
    .min(1)
    ...
    .optional()
    .describe(
      'Which fields to include in the response. Defaults vary by entities. Common fields: id, area, length, radius, axis, diameter, extent_along_axis, surface_type, curve_type, normal, bbox, bbox_center, validity_status, tolerance, face_ids, value, datum_refs, linked_to, pos_hits, neg_hits. The LLM can omit this to get defaults.',
    ),
  sort: z
    .object({
  ...
  offset: z
    ...
    .min(0)
    .default(0)
    .describe('Skip this many results before returning. Default 0.'),
  return_type: z
    .enum(RETURN_TYPES)
    .default('entities')
    .describe(
      'Result shape: "entities" (default) returns paginated entities, "summary" returns statistics only, "groups" returns group counts with sample IDs (requires group_by).',
    ),
};
  ...
  z
    .object({
      ...
      params: z
        ...
        .optional()
        .describe(
          'For "query": a query_step input shape (entities, filter, group_by, etc.). For "walk_assembly": {per_node: [sub-pipeline]}.',
        ),
      do: z
      ...
      where: z
        ...
        .optional()
        .describe(
          'For "filter_results": a simple expression. Format: "field op value" or "field.empty" or "field.count op value". Examples: "diameter > 5", "pos_hits.empty", "face_ids.count == 1".',
        ),
      fields: z
        .array(z.string())
        .min(1)
...
export const transactStepInputSchema = {
  ...
  pipeline: z
    ...
    .max(50)
    .describe(
      "Ordered list of pipeline operations. Each op reads from the previous op's output by default. Common ops: query, for_each, filter_results, select, walk_assembly.",
    ),
  return_intermediate: z
    .boolean()
    .default(false)
    .describe(
      'If true, include the result of each step in the response. Default false; set true only for debugging pipelines.',
    ),
};
